...
def testmltechniques():
    ...
    for i in range(1,6):
        ...
        test_data,test_labels,results,fi, blind_results = MLTechniques.VGG16_CNN.train(data)
        assert (True == (test_data is not None and test_labels is not None and results is not None and fi is None and blind_results is None)),"Algorithm error"       
    ...
    for i in range(1,6):
        ...
        test_data,results = MLTechniques.KMeans.train(data)
        assert (True == (test_data is not None and results is not None)),"Algorithm error"
        
        test_data,results = MLTechniques.HAC.train(data)
        assert (True == (test_data is not None and results is not None)),"Algorithm error"
        
        test_data,results = MLTechniques.MeanShift.train(data)
        assert (True == (test_data is not None and results is not None)),"Algorithm error"
        
        test_data,results = MLTechniques.DBSCAN.train(data)
        assert (True == (test_data is not None and results is not None)),"Algorithm error"
        
        test_data,results = MLTechniques.Autencoder.train(data)
        assert (True == (test_data is not None and results is not None)),"Algorithm error"
